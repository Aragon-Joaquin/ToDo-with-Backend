...
export function TabsHelper() {
	return (
		<Tabs defaultValue={AVAILABLE_OPTIONS[0].valueName} className="my-10 rounded-lg h-fit w-1/2 mx-auto">
			<TabsList className="w-full flex flex-row justify-around items-center bg-zinc-900 border-b-2 border-stone-100 rounded-t-md font-medium">
				{AVAILABLE_OPTIONS.map(({ valueName, informationTab }) => (
					<TabsTrigger
						key={valueName}
						value={valueName}
						className=" data-[state=active]:bg-zinc-700 rounded w-full px-4 py-2 transition-all"
					>
						{informationTab}
					</TabsTrigger>
				))}
